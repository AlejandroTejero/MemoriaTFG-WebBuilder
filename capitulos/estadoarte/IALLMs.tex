...
Como comenté en el párrafo anterior, la calidad de la generación depende en gran medida del prompt, pero una parte tambien importante es la potencia o calidad del LLM con el que estemos trabajando. Si el LLM no tiene potencia o esta poco entrenado, aun teniendo un buen prompt este modelo no tendrá suficiente contenido como para generar la respuesta que buscamos.
...
Los tres proveedores descritos comparten el formato OpenAI-compatible~\cite{openai_api_docs}, 
un estándar de facto que define un endpoint \texttt{/chat/completions} el cual acepta 
una lista de mensajes y devuelve la respuesta del modelo. Esto permite a WebBuilder 
comunicarse con cualquiera de ellos cambiando únicamente la URL base, la clave de API 
y el identificador del modelo, sin necesidad de modificar la lógica interna del sistema.