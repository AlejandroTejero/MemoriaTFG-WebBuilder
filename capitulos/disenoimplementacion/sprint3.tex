\section{Sprint 3: Integración LLM y n8n}
\label{sec:integracion-llm-n8n}

Con el sprint 2 terminado y viendo que las limitaciones eran evidentes, se tomó la decisión de realizar un cambio fundamental para el futuro del proyecto. Se abandonó 
...
Un ejemplo de análisis en la primera interfaz de WebBuilder es el siguiente: 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/oldversions/primerschemaia}
    \caption{Primer schema generado automáticamente por el LLM}
    \label{fig:primerschemaia}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/oldversions/primeredit}
    \caption{Primera pantalla de edición del schema propuesto por el LLM}
    \label{fig:primeredit}
\end{figure}
...
A continuación se muestra el prompt de sistema y el prompt de usuario que se construye 
y se envía al LLM en esta fase:

\begin{lstlisting}[caption={Prompt de sistema del planner}, label={lst:planner-system}]
Eres un planificador ESTRICTO para una app que genera sitios web a 
partir de datasets JSON/XML. Tu respuesta DEBE ser SOLO un único 
objeto JSON válido. NO uses Markdown. NO uses texto extra. 
NO uses bloques ```. SOLO devuelve el JSON.
\end{lstlisting}

\begin{lstlisting}[caption={Prompt de usuario del planner (estructura general)}, label={lst:planner-user}]
OBJETIVO_USUARIO (texto libre):
<descripción en lenguaje natural introducida por el usuario>
...
REGLAS OBLIGATORIAS:
- Devuelve SOLO un objeto JSON válido. Sin texto extra. Sin Markdown.
...
- 'key' DEBE ser exactamente una de las strings de available_keys.
- PROHIBIDO en 'key': valores del dataset, rutas o texto inventado.

available_keys (SOLO estas strings son válidas para el campo 'key'):
["campo1", "campo2", "campo3", ...]

examples (1-3 items reales del dataset):
[
...
cualquier texto adicional produce fallo y rompe el proceso de parseo.
...
\begin{itemize}
    ...
    realidad es una etiqueta del propio prompt.
