...
\begin{itemize}
    ...
    versión mínima pero funcional de un fichero concreto del proyecto: 
...
La home del proyecto, que hasta entonces era el fichero más complejo, se dividió en seis partials independientes. Aprovechando el momento también se divido el fichero assistant, en este caso tres partials. En la Tabla~\ref{tab:partials-html} se muestra la estructura completa de partials tras el refactor:
...
El historial también se dividió en dos partials:
...
\begin{table}[H]
    ...
    \textbf{Partial} & \textbf{Página} & \textbf{Responsabilidad} \\
    \hline
        published\_sites.html & history & Listado de sitios generados y desplegados por el usuario \\
    \hline
        recent\_analysis.html & history & Listado de análisis recientes realizados por el usuario \\
    \hline
    \end{tabular}
    \caption{Partials HTML del historial tras el refactor de templates}
    \label{tab:partials-history}
\end{table}

Los ficheros estáticos sufrieron un refactor equivalente.
El CSS pasó a estar organizado por página y por sección dentro de
\texttt{static/css/}, separando los estilos de la página principal
en una subcarpeta \texttt{home/} con un fichero por sección, y
manteniendo un fichero independiente para cada vista principal de
la aplicación: el asistente, el historial, la edición del schema,
el login y la barra de navegación, además de un fichero compartido
para fondos y efectos visuales globales.

El JavaScript siguió el mismo criterio, con ficheros independientes
por página en \texttt{static/js/}: uno para la lógica interactiva
de la home, otro para el flujo del asistente, otro para la edición
del schema y un último para la vista previa del sitio generado.
...
el esquema generado por la IA a la derecha, 
...
Si el dataset tenía campos con imágenes se le indica al modelo que las usara de forma destacada. Si tenía precios, que los resalte visualmente. Si tenía
...
Otro problema frecuente era que el LLM no tenía suficiente contexto entre ficheros, principal causa de las alucinaciones del LLM. Ocurría lo siguiente:
