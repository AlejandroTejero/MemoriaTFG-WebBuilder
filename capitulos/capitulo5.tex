...
\subsection{Caso de uso 2: Catálogo de películas de animación}
\label{subsec:caso-peliculas}

El segundo caso de uso utiliza la API pública de SampleAPIs, que devuelve un catálogo de películas de animación con imagen, título, año e identificador de IMDB. El endpoint utilizado fue:

\begin{center}
\texttt{https://api.sampleapis.com/movies/animation}
\end{center}

El prompt introducido por el usuario fue el siguiente:

\begin{quote}
\textit{Genera una web de catálogo de películas de animación. Muestra todas las películas en una cuadrícula con su título, año de lanzamiento y puntuación. Cada película debe tener su propia página de detalle. La página principal debe tener una sección hero de bienvenida y una sección con las películas mejor valoradas. Usa un estilo moderno y colorido.}
\end{quote}

El sistema detectó automáticamente los campos disponibles en la API y el LLM propuso un schema de tipo \texttt{catalog} con el título \textit{Catálogo de Películas de Animación}. La generación completó los siete pasos sin errores ni reintentos.

El sitio resultante incluye una página principal con una sección de bienvenida y una cuadrícula de películas mejor valoradas con sus pósters, un catálogo completo con todas las películas disponibles organizado en cuadrícula de cuatro columnas y páginas de detalle individuales para cada película.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/casosdeuso/peliculas/peliculashome.png}
    \caption{Página principal del catálogo de películas de animación generado por WebBuilder}
    \label{fig:peliculas-home}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/casosdeuso/peliculas/peliculascatalogo.png}
    \caption{Catálogo completo de películas de animación con pósters e información}
    \label{fig:peliculas-catalogo}
\end{figure}

En cuanto a la evaluación del caso, el sitio funciona correctamente: todas las páginas cargan, los datos de la API se muestran bien y no hubo errores durante la generación. El catálogo visual con pósters queda especialmente bien dado que la API devuelve URLs de imagen directas. Como limitación destacable, el campo de puntuación no contiene una valoración numérica real sino el identificador de IMDB de cada película, lo que hace que aparezca como \textit{Puntuación IMDB: tt7424200} en lugar de una nota. Esto es una limitación de los datos de la propia API y no del sistema de generación. El despliegue automático funcionó correctamente y el contenedor quedó accesible mediante su URL de previsualización.
